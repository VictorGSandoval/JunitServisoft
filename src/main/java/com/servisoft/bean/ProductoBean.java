...
public class ProductoBean implements Serializable {
	...
	public static void validarProducto(ComponentSystemEvent event) {
		try {
			FacesContext fc = FacesContext.getCurrentInstance();
			UIComponent component = event.getComponent();
			UIInput uiProd = (UIInput) component.getAttributes().get("txtNOMPROD");
			String idProd = uiProd.getClientId();
			UIInput uiTam = (UIInput) component.getAttributes().get("txtTAMPROD");
			String idTam = uiTam.getClientId();
			
			System.out.println(idProd + "  " + idTam);
			
			String sql = "SELECT NOMPRO, TAMPRO FROM PRODUCTO WHERE NOMPRO = '"+ idProd +"' AND TAMPRO = '"+ idTam +"'";
			...
			while(rs.next()) {
				...
				String tamano = rs.getString("2");
				System.out.println("El producto "+ producto + " con el tamaño "+ tamano + " si existen");
			}
			rs.close();
		} catch (Exception e) {
